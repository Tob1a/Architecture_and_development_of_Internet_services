...
int rxb_readline(struct rx_buffer *buf, int sock, void *dest_buf, size_t *dest_len)
{
        ...
        do
        {
                ...
                cc = read(sock, buf->buffer + buf->bytes_received, left); // il secondo parametro è il puntatore del buffered dove vado a mettere quello che leggo. il terzo è il numero di byte che mi manca da leggere
                if (cc == 0)
                        return -1;
                if (cc < 0)
                {
...
{
        int cc = read(sock, buf->buffer, buf->size);
        if (cc == 0)
                return -1;
        if (cc < 0)
        {
